...
if (unenriched.length > 0) {
	console.error(
		`${unenriched.length} chunk(s) still have an empty "kana" or "translation" field — chunks.json is generated by ingest:cut from chunk_plan.json, don't edit it by hand. Fill those fields in chunk_plan.json and re-run ingest:cut. Affected: ${unenriched.map((c) => c.audioFile).join(', ')}`
	);
	process.exit(1);
}
